...
-- Demo inits -----------------------------------------------------------------
local space_new = cp.space_new
local bevel = 1.0

math.randomseed(os.time())

local function rand_unit_circle() -- random point in unit circle
   while true do -- sample a random point in square until its magnitude is less than 1
      local v = vec2(math.random()*2-1, math.random()*2,-1)
      if v:norm2()<1 then return v end
   end
end

local function rand_size()
   return 1.5^mix(-1.5, 3.5, math.random())
end

local function add_circle(space, radius)
   local mass = radius*radius/25.0
   local body = space:add_body(cp.body_new(mass, cp.moment_for_circle(mass, 0.0, radius, {0, 0})))
   body:set_position(rand_unit_circle()*180.0)
   local shape = space:add_shape(cp.circle_shape_new(body, radius, {0, 0}))
   shape:set_elasticity(0.0) shape:set_friction(0.9)
end

local function add_box(space, size)
   local mass = size*size/100.0
   local body = space:add_body(cp.body_new(mass, cp.moment_for_box(mass, size, size)))
   body:set_position(rand_unit_circle()*180.0)
   local shape = space:add_shape(cp.box_shape_new(body, size - bevel*2, size - bevel*2, 0.0))
   shape:set_radius(bevel)
   shape:set_elasticity(0.0) shape:set_friction(0.9)
end

local function add_hexagon(space, radius)
   local hexagon = {}
   for i=0, 5 do
      local angle = -pi*2.0*i/6.0
      table.insert(hexagon, vec2(cos(angle), sin(angle))*(radius - bevel))
   end
   local mass = radius*radius
   local body = space:add_body(cp.body_new(mass, cp.moment_for_poly(mass, hexagon, {0, 0}, 0.0)))
   body:set_position(rand_unit_circle()*180.0)
   local shape = space:add_shape(cp.poly_shape_new(body, hexagon, bevel, identity))
   shape:set_elasticity(0.0) shape:set_friction(0.9)
end


local function SetupSpace_simpleTerrain()
   local space = space_new()
   space:set_iterations(10)
   space:set_gravity({0, -100})
   space:set_collision_slop(0.5)
   local offset = vec2(-320, -240)
   for i=1, #simple_terrain_verts-1 do
      local a, b = simple_terrain_verts[i], simple_terrain_verts[i+1]
      a, b = vec2(a[1], a[2]), vec2(b[1], b[2])
      space:add_shape(cp.segment_shape_new(space:get_static_body(), a+offset, b+offset, 0.0))
   end
   return space
end
...
local function init_SimpleTerrainCircles(n)
   local space = SetupSpace_simpleTerrain()
   for i=1, n do add_circle(space, 5.0) end
   return space
end

local function init_SimpleTerrainCircles_1000() return init_SimpleTerrainCircles(1000) end
local function init_SimpleTerrainCircles_500() return init_SimpleTerrainCircles(500) end
local function init_SimpleTerrainCircles_100() return init_SimpleTerrainCircles(100) end

local function init_SimpleTerrainBoxes(n)
   local space = SetupSpace_simpleTerrain()
   for i=1, n do add_box(space, 10.0) end
   return space
end

local function init_SimpleTerrainBoxes_1000() return init_SimpleTerrainBoxes(1000) end
local function init_SimpleTerrainBoxes_500() return init_SimpleTerrainBoxes(500) end
local function init_SimpleTerrainBoxes_100() return init_SimpleTerrainBoxes(100) end

local function init_SimpleTerrainHexagons(n)
   local space = SetupSpace_simpleTerrain()
   for i=1, n do add_hexagon(space, 5.0) end
   return space
end

local function init_SimpleTerrainHexagons_1000() return init_SimpleTerrainHexagons(1000) end
local function init_SimpleTerrainHexagons_500() return init_SimpleTerrainHexagons(500) end
local function init_SimpleTerrainHexagons_100() return init_SimpleTerrainHexagons(100) end

-- SimpleTerrain variable sized objects

local function init_SimpleTerrainVCircles_200()
   local space = SetupSpace_simpleTerrain()
   for i=1, 200 do add_circle(space, 5.0*rand_size()) end
   return space
end

local function init_SimpleTerrainVBoxes_200()
   local space = SetupSpace_simpleTerrain()
   for i=1, 200 do add_box(space, 8.0*rand_size()) end
   return space
end

local function init_SimpleTerrainVHexagons_200()
   local space = SetupSpace_simpleTerrain()
   for i=1, 200 do add_hexagon(space, 5.0*rand_size()) end
   return space
end

local function init_ComplexTerrainCircles_1000()
   local space = space_new()
   space:set_iterations(10)
   space:set_gravity({0, -100})
   space:set_collision_slop(0.5)
   local offset = vec2(-320, -240)
   for i=1, #complex_terrain_verts - 1 do
      local a, b = complex_terrain_verts[i], complex_terrain_verts[i+1]
      a, b = vec2(a[1], a[2]), vec2(b[1], b[2])
      space:add_shape(cp.segment_shape_new(space:get_static_body(), a+offset, b+offset, 0.0))
   end
   for i=1, 1000 do
      ...
      local mass = radius*radius
      local body = space:add_body(cp.body_new(mass, cp.moment_for_circle(mass, 0.0, radius, {0, 0})))
      body:set_position(rand_unit_circle()*180.0 + vec2(0.0, 300.0))
      local shape = space:add_shape(cp.circle_shape_new(body, radius, {0, 0}))
      shape:set_elasticity(0.0) shape:set_friction(0.0)
   end
   return space
end

local function init_ComplexTerrainHexagons_1000()
   local space = space_new()
   space:set_iterations(10)
   space:set_gravity({0, -100})
   space:set_collision_slop(0.5)
...
local function init_BouncyTerrainCircles_500()
   local space = space_new()
   space:set_iterations(10)
   local offset = vec2(-320, -240)
   for i=1, #bouncy_terrain_verts - 1 do
      local a, b = bouncy_terrain_verts[i], bouncy_terrain_verts[i+1]
      a, b = vec2(a[1], a[2]), vec2(b[1], b[2])
      local shape = space:add_shape(cp.segment_shape_new(space:get_static_body(), a+offset, b+offset, 0.0))
   ...
   for i=1, 500 do
      ...
      local mass = radius*radius
      local body = space:add_body(cp.body_new(mass, cp.moment_for_circle(mass, 0.0, radius, {0, 0})))
      body:set_position(rand_unit_circle()*130.0+vec2(0, 0))
      body:set_velocity(rand_unit_circle()*50.0)
      local shape = space:add_shape(cp.circle_shape_new(body, radius, {0, 0}))
      shape:set_elasticity(1.0)
   end
   return space
...
local function init_BouncyTerrainHexagons_500()
   local space = space_new()
   space:set_iterations(10)
   local offset = vec2(-320, -240)
   for i=1, #bouncy_terrain_verts - 1 do
...
local function init_NoCollide()
   local space = space_new()
   space:set_iterations(10)
   local handler = space:add_collision_handler(2, 2)
   handler:set_begin_func(function(arbiter, space)
      error("This callback should never be called")
      return true
   end)
   local r= 4.5 -- radius
   local static_body = space:get_static_body()
   local shape = cp.segment_shape_new(static_body, {-330-r, -250-r}, { 330+r, -250-r}, 0.0)
   ...
   for x=-320, 320, 20 do
      for y=-240, 240, 20 do
         space:add_shape(cp.circle_shape_new(static_body, r, {x, y}))
      end
   end
   for y=10-240, 240, 40 do
      local mass = 7.0
      local body = space:add_body(cp.body_new(mass, cp.moment_for_circle(mass, 0.0, r, {0, 0})))
      body:set_position({-320.0, y})
      body:set_velocity({100.0, 0.0})
      local shape = space:add_shape(cp.circle_shape_new(body, r, {0, 0}))
      shape:set_elasticity(1.0)
      shape:set_collision_type(2)
   end
   for x=30-320, 320, 40 do
      local mass = 7.0
      local body = space:add_body(cp.body_new(mass, cp.moment_for_circle(mass, 0.0, r, {0, 0})))
      body:set_position({x, -240.0})
      body:set_velocity({0.0, 100.0})
      local shape = space:add_shape(cp.circle_shape_new(body, r, {0, 0}))
      shape:set_elasticity(1.0)
      shape:set_collision_type(2)
   end
...
local demo_list = {
   [1] = { init = init_SimpleTerrainCircles_1000, title = "SimpleTerrainCircles_1000" },
   [2] = { init = init_SimpleTerrainCircles_500, title = "SimpleTerrainCircles_500" },
   [3] = { init = init_SimpleTerrainCircles_100, title = "SimpleTerrainCircles_100" },
   [4] = { init = init_SimpleTerrainBoxes_1000, title = "SimpleTerrainBoxes_1000" },
   [5] = { init = init_SimpleTerrainBoxes_500, title = "SimpleTerrainBoxes_500" },
   [6] = { init = init_SimpleTerrainBoxes_100, title = "SimpleTerrainBoxes_100" },
   [7] = { init = init_SimpleTerrainHexagons_1000, title = "SimpleTerrainHexagons_1000" },
   [8] = { init = init_SimpleTerrainHexagons_500, title = "SimpleTerrainHexagons_500" },
   [9] = { init = init_SimpleTerrainHexagons_100, title = "SimpleTerrainHexagons_100" },
   [10] = { init = init_SimpleTerrainVCircles_200, title = "SimpleTerrainVCircles_200" },
   [11] = { init = init_SimpleTerrainVBoxes_200, title = "SimpleTerrainVBoxes_200" },
   [12] = { init = init_SimpleTerrainVHexagons_200, title = "SimpleTerrainVHexagons_200" },
   [13] = { init = init_ComplexTerrainCircles_1000, title = "ComplexTerrainCircles_1000" },
   [14] = { init = init_ComplexTerrainHexagons_1000, title = "ComplexTerrainHexagons_1000" },
   [15] = { init = init_BouncyTerrainCircles_500, title = "BouncyTerrainCircles_500" },
   [16] = { init = init_BouncyTerrainHexagons_500, title = "BouncyTerrainHexagons_500" },
   [17] = { init = init_NoCollide, title = "NoCollide" },
}

-- Command line args:
local demo_number = arg[1] and tonumber(arg[1]) or 16
local demo = demo_list[demo_number]
print(demo_number, demo)
if not demo then
   error("invalid demo number (must be between 1 and 17 inclusive)")
end

if arg[2]=='hasty' then space_new = cp.hasty_space_new end

TITLE = demo.title
local space = demo.init()
